...
export function DesktopSidebar({ userEmail }: { userEmail: string | null }) {
  ...
  return (
    ...
    >
      ...
      >
        {collapsed ? (
          <Link
            ...
            title={t("appName")}
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-md border border-[var(--sidebar-border)] bg-white text-[var(--sidebar-text)] shadow-sm">
      ...
      <SidebarUserSection userEmail={userEmail} collapsed={collapsed} />
    </aside>
  );
}
